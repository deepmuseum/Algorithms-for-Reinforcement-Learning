...
class LinearMABModel(object):
    ...
    def reward(self, action):
        assert 0 <= action < self.n_actions, "{} not in 0 .. {}".format(
            action, self.n_actions
        )
        reward = np.dot(
            self.features[action], self.theta
        ) + self.noise * self.local_random.randn(1)
...
class RandomLinearArms(LinearMABModel):
    def __init__(
        ...
        real_theta = (
            np.random.uniform(low=1 / 2, high=bound_theta)
            * real_theta
            / np.linalg.norm(real_theta)
        )
        if positive:
            ...
            for i in idxs:
                ...
                features[i, :] = feat
        features = (
            np.random.uniform(low=1 / 2, high=bound_features, size=(n_actions, 1))
            * features
            / max(np.linalg.norm(features, axis=1))
        )
        ...
        super(RandomLinearArms, self).__init__(
            random_state=random_state, noise=noise, features=features, theta=real_theta
        )


class ColdStartFromDataset(LinearMABModel):
    def __init__(
        self, arm_csvfile, user_csvfile, random_state=0, noise_std=0.0, user_subset=None
    ):
        features = np.loadtxt(arm_csvfile, delimiter=",").T
        thetas = np.loadtxt(user_csvfile, delimiter=",")

        super(ColdStartFromDataset, self).__init__(
            random_state=random_state, noise=noise_std, features=features, theta=None
        )
